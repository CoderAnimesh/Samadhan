...
function ComplaintCard({ complaint, onComplete }) {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (isNaN(compLat) || isNaN(compLng)) {
          toast.error('Complaint location is invalid.');
          return;
        }
        ...
        if (distance <= 3) {
          ...
          toast.error(`Verification Failed! You are ${Math.round(distance)} meters away. You must be within 3 meters.`);
        }
      },
      (err) => {
        setLoadingLoc(false);
  ...
  const handleConfirmResolution = async () => {
    setVerifying(true);
    try {
      await api.patch(`/workers/complete-complaint/${complaint.id}`);
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ fontSize: '0.82rem', color: '#94a3b8', marginTop: 4 }}>
            Reported on {new Date(complaint.createdAt).toLocaleDateString('en-IN')}
          </p>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.85rem', color: '#a5b4fc', background: 'rgba(99,102,241,0.06)', padding: '10px 14px', borderRadius: 10 }}>
        <MapPin size={16} />
        {complaint.address || complaint.area || 'Location attached'}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 'auto' }}>
        ...
        >
          {loadingLoc ? <><Loader size={16} className="spin" /> Verifying GPS…</> : verifying ? 'Marking…' : <><CheckCircle size={16} /> Mark Resolved</>}
        </button>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 800, fontSize: '1.05rem', lineHeight: 1 }}>SAMADHAN</div>
            <div style={{ fontSize: '0.65rem', color: '#10b981', letterSpacing: 1, fontWeight: 700, marginTop: 2 }}>WORKER PANEL</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '0.7rem', color: '#10b981', fontWeight: 600 }}>Active Worker</div>
            </div>
      ...
      <main style={{ flex: 1, padding: '40px 5%', maxWidth: 1400, margin: '0 auto', width: '100%' }}>
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 700, fontSize: '2rem' }}>Your Assignments</h1>
          <p style={{ color: '#94a3b8', fontSize: '0.95rem', marginTop: 4 }}>
            Go to the location, complete the work, and resolve constraints via GPS.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state" style={{ height: 400 }}>
            ...
            <h2 style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 700, fontSize: '1.4rem' }}>You're all caught up!</h2>
            <p style={{ color: '#94a3b8', maxWidth: 300, fontSize: '0.9rem' }}>You have no pending complaints assigned right now.</p>
          </div>
        ) : (
