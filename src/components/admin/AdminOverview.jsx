...
export default function AdminOverview({ onNavigate }) {
  ...
  const stats = [
    ...
    { label: 'Pending Review', value: counts.pending, icon: Clock, color: '#f59e0b', bg: 'amber' },
    { label: 'Active Workers', value: workers.filter(w => w.isAvailable).length, icon: Users, color: '#22d3ee', bg: 'blue' },
      ...
      {counts.pending > 0 && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, color: '#f59e0b' }}>{counts.pending} complaint{counts.pending > 1 ? 's' : ''} awaiting review</div>
              <div style={{ fontSize: '0.82rem', color: '#94a3b8' }}>Click to review and assign workers</div>
            </div>
          </div>
        ...
        <div className="table-container">
          <table className="table">
            ...
            <tbody>
              ...
              {recent.length === 0 && (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '32px', color: '#475569' }}>No complaints yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
